...
export const finalAccept = async (req, res) => {
	...
	for (let i=0; i<2; i++) {	// 구매자, 판매자 최종 안내 이메일 보내기
		...
		if (i === 0) {	// 구매자에게
			email_body = emailTempleteNotification(buyer.name, `http://116.42.123.180/main/transaction/${pk}`, "전력 거래 진행 안내",
			...
			email_body = emailTempleteNotification(seller.name, `http://116.42.123.180/main/transaction/${pk}`, "전력 거래 진행 안내",
